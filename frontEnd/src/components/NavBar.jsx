...
const NavBar = () => {
  ...
  const {setShowSearch, getCartCount, navigate, token, setToken, setCartItems, userD} = useContext(ShopContext)
  ...
  return (
    <div className="flex items-center justify-between py-5 font-medium">
      <Link to='/'><img src={assets.logo2} className="w-36" /></Link> 
      ...
      <div className="flex items-center gap-6">
        ...
        <img onClick={()=>setVisible(true)} src={assets.menu_icon} alt="" className="w-5 curor-pointer sm:hidden" />
      </div>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full':'w-0'}` }>
        <div className="flex flex-col text-gray-600">
          <div onClick={()=>setVisible(false)} className="flex items-center gap-4 p-3 cursor-pointer" >
            <img src={assets.dropdown_icon} alt="" className="h-4 rotate-180" />back
          </div>
